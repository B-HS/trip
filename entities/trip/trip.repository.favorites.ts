...
export const setTripFavorite = async (userId: string, tripId: string, isFavorite: boolean) => {
    const db = getDb()
    if (!isFavorite) {
        await db.delete(tripFavorite).where(and(eq(tripFavorite.userId, userId), eq(tripFavorite.tripId, tripId)))
        return
    }
    const [last] = await db
    ...
    await db.insert(tripFavorite).values({ userId, tripId, sortOrder }).onDuplicateKeyUpdate({ set: { tripId } })
}
