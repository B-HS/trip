...
import { getCachedTripDetail, getCachedTripList } from '@/entities/trip/trip.cache'
...
export const prefetchTripList = async (queryClient: QueryClient, userId: string) =>
    queryClient.prefetchQuery({ queryKey: QUERY_KEY.TRIP.LIST, queryFn: () => getCachedTripList(userId) })
...
export const prefetchTripDetail = async (queryClient: QueryClient, tripId: string, userId: string) =>
    queryClient.prefetchQuery({
        ...
        queryFn: async () => {
            const [detail, viewerRole] = await Promise.all([getCachedTripDetail(tripId), getTripRole(tripId, userId)])
