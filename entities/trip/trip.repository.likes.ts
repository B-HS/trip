...
export const setTripLike = async (tripId: string, userId: string, liked: boolean) => {
    const likeCount = await getDb().transaction(async (tx) => {
        const [target] = await tx.select({ id: trip.id }).from(trip).where(eq(trip.id, tripId)).limit(1).for('update')
        ...
        await tx
            .update(trip)
            .set({ likeCount: value, updatedAt: new Date(), revision: sql`${trip.revision} + 1` })
