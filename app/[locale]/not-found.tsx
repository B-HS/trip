        ...
        <div className='surface-public flex flex-1 flex-col bg-background'>
            <Empty className='gap-8 px-6 py-16'>
                <EmptyHeader className='max-w-md gap-3'>
                    ...
                    <span className='text-2xs font-medium tracking-wide text-muted-foreground'>{t('eyebrow')}</span>
                    <h1 className='text-xl font-extrabold tracking-tight text-balance'>{t('title')}</h1>
                    <EmptyDescription className='leading-7'>{t('description')}</EmptyDescription>
                </EmptyHeader>
                <EmptyContent className='w-fit flex-row flex-wrap items-stretch justify-center gap-px bg-background'>
                    <Button asChild variant='cellPrimary' size='cell'>
                        <Link href={NOT_FOUND_COPY.primaryAction.href}>{t('goHome')}</Link>
                    </Button>
                    <Button asChild variant='cell' size='cell'>
                        <Link href={NOT_FOUND_COPY.secondaryAction.href}>{t('myTrips')}</Link>
                    </Button>
                </EmptyContent>
            </Empty>
        </div>
