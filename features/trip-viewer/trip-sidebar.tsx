        ...
        <aside className={cn('flex flex-col gap-3 p-3 text-foreground', isPrintLayout && 'break-inside-avoid')}>
            ...
            <div className='text-sm text-foreground'>
                <p className='font-mono tabular-nums'>
                    ...
                    {LENGTH_SEPARATOR}
                    {formatTripLength({ startDate, endDate, nights: customNights, days: customDays }, locale)}
                </p>
            ...
            <div className='flex flex-col gap-1'>
                <span aria-live='polite' className='text-xs text-muted-foreground'>
                    {t('sidebarCompleted', { ratio: formatRatio(completedCount, totalCount) })}
                </span>
                <AnimatedProgress value={toPercent(completedCount, totalCount)} label={t('sidebarProgressAria')} />
            </div>
            {destinations.length > 0 && (
                <section className='flex flex-col gap-2'>
                    <h2 className='text-sm font-medium text-foreground'>{t('sidebarDestinations')}</h2>
                    <ol className='flex flex-col gap-1'>
                        {destinations.map((destination, index) => (
                            <li key={`${index}-${destination.countryCode}`} className='flex items-baseline gap-2 text-sm'>
                                ...
                                <span className='truncate'>{destination.city ?? countryName(destination.countryCode, locale)}</span>
                            </li>
                        ))}
                    </ol>
                </section>
            ...
            {flights.length > 0 && (
                <section className='flex flex-col gap-2'>
                    <h2 className='text-sm font-medium text-foreground'>{t('sidebarConfirmedFlights')}</h2>
                    {flights.map((flight) => (
            ...
            {lodgings.length > 0 && (
                <section className='flex flex-col gap-2'>
                    <h2 className='text-sm font-medium text-foreground'>{t('sidebarDepartures')}</h2>
            ...
            {sidebarLinks.length > 0 && (
                <section className='flex flex-col gap-2'>
                    <h2 className='text-sm font-medium text-foreground'>{t('sidebarLinks')}</h2>
            ...
            {(disclaimer || verifiedOn) && (
                <p className='text-xs text-muted-foreground'>
                    ...
                    {verifiedOn && t('sidebarVerifiedOn', { date: formatVerifiedOn(verifiedOn) })}
                </p>
            )}
        </aside>
    )
