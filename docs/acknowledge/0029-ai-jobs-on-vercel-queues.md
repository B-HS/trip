...
- 사용자 결정(세션 2 후반): **키 없이 먼저 구현**한다. `APP_ENCRYPTION_KEY` 가 없거나 유효하지 않으면 AI capability를 끄고 AI 메뉴·트립 도우미 UI를 렌더링하지 않으며 인증된 `/settings/ai` 직접 접근은 404로 처리한다(비로그인 요청은 보호 경계에서 로그인으로 이동). 사용자가 `.env` 와 Vercel 에 유효한 키를 넣은 뒤 provider 키 등록을 테스트한다. `.env.example` 에는 키 이름만 둔다.
