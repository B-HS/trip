...
- [Vercel Queues API](https://vercel.com/docs/queues/api) documents the v3 topic endpoint and bearer OIDC authentication. The REST fallback uses `VERCEL_QUEUE_URL` and `VERCEL_QUEUE_TOKEN`; the callback route also requires the same bearer token.
