...
- `trip_ai_key` stores only ciphertext, IV, GCM tag, and the final-four hint. `APP_ENCRYPTION_KEY` is optional at application startup, but it is the global AI capability prerequisite: only a valid base64 32-byte value enables AI. When it is absent or invalid, AI navigation and trip-assistant UI are omitted and authenticated `/settings/ai` access returns 404 (anonymous requests follow the existing protected-route login redirect). Provider API keys remain user-owned database records and are never required in process env.
- The server evaluates this capability once in the shell/page boundaries and passes only a boolean into client components. No environment value or provider secret is included in the client bundle. Queue credentials are not part of the capability predicate because local/test execution has a deterministic fallback; production uses the configured Vercel Queues trigger/OIDC boundary.
...
- Job execution is at-least-once with a durable running lease. The output transaction locks and revalidates the job lease before inserting the assistant message, usage, or proposal; unique `job_id` markers make a retried output a no-op. Proposal application has its own fenced lease and captures the monotonic trip `revision` CAS baseline so an expired worker cannot overwrite a newer edit.
...
- A durable `trip_ai_dispatch` outbox row is committed with every job. The initial publish may fail without losing the job; the secured `/api/cron/ai-dispatch` Vercel cron retries leased rows with bounded backoff and also prunes expired developer-API idempotency records. Set `CRON_SECRET` for the cron invocation.
