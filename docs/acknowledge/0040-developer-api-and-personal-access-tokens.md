# ADR-0040 — Versioned developer API and personal access tokens (2026-09-12)
...
`0010_trip-consent.sql`이 아직 운영 DB에 적용되지 않은 기준에서 이 마이그레이션은 `0011`로 provision한다. 통합자는 Phase 7/6의 스냅샷·journal 순서를 확인한 뒤 한 번만 생성·적용하고, 멀티 인스턴스 배포 전 idempotency/rate-limit 캐시를 공유 저장소로 교체해야 한다.
