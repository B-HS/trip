...
> 최종 갱신: 2026-09-11 · 대응 커밋: `181ab9c`(4-5 커뮤니티 확장 데이터 계층 — ADR-0037, 마이그레이션 0008 생성·SQL 검증, **DB 미적용**: 이 환경에 `DATABASE_URL` 없음)

> 2026-09-09 사용자가 기획 중인 고도화 기능. 여기 적힌 제약은 구현 시 그대로 지킨다. 착수 범위·순서·정책 답변은 ADR-0022.

## 진행 상태 (2026-09-10 세션 4 4-4e 기준)
...
| 4 AI                                        | 구현 완료(운영 설정 대기)                 | ADR-0029·0040 — 자기 키 AES-256-GCM, 동적 공식 모델 목록·추론 검증, 영속 대화/작업/사용량, Queues callback, 승인형 일정 diff                                                     |
| 5 일정 종류 배지                            | 완료                                      | ADR-0025, 마이그레이션 0004(데이터 이관), 편집기 "일정 종류" 탭                                                                                                                  |
| 6 커뮤니티·프로필                           | 확장 데이터 계층 완료(4-5) / 확장 UI 대기 | ADR-0028·0032 기본 + ADR-0037 확장(소프트 삭제·신고·차단·채택 회수·사용자명 변경·첨부 축소) 데이터 계층 완료(마이그레이션 0008 생성, DB 미적용). UI 는 후속 에이전트             |
...
- **SEO**: 라우트별 `generateMetadata`(title template·description·canonical·robots), `app/sitemap.ts`(정적 페이지 + 공개 트립 `/s/[slug]`), `app/robots.ts`, Open Graph·Twitter 카드(공개 트립은 `app/[locale]/(shell)/s/[slug]/opengraph-image.tsx`, Twitter는 같은 폴더의 `twitter-image.tsx` 로 동적 카드 — 제목·기간·목적지), `hreflang`/`lang='ko'` 유지, 시맨틱 헤딩 계층 점검, 이미지 `alt`.
...
- 2026-09-10 세션 4 에서 추가된 항목(원래 로드맵에 없던 사용자 요청). 결정·구현·실측 기록은 ADR-0035 가 정본이다.
- 출발지는 트립별 값이다: `trip_trip.departure_airport_code`(마이그레이션 0007, NULL 이면 `HOME_AIRPORT_CODE` ICN). 편집기 "기본 정보" 탭 콤보박스(43개 공항, 비우기 = ICN), 템플릿 JSON 선택 필드로 왕복.
- `/trips` 헤더 지구본은 드래그 회전(정밀 포인터 한정 OrbitControls), 선 hover 툴팁(코드·도시 + 트립 제목·기간), 선 클릭 시 `?route=` 로 그 경로의 트립만 필터한다. 인트로 히어로·빈 상태·404 지구본은 기존 동작 유지.
- 남은 제약: 포인터 없이 필터를 걸 수 없다(ADR-0035 §7). 경로 목록을 셀 버튼으로 노출하는 방식은 후속 판단.
