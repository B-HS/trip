...
> 최종 갱신: 2026-09-11 · 대응 커밋: `181ab9c`(4-5 커뮤니티 확장 **데이터 계층** — ADR-0037 · 마이그레이션 0008 생성·추가 전용 SQL 검증 완료, **DB 미적용**: 이 환경에 `.env`/`DATABASE_URL` 없음. `bun run db:migrate` 는 사용자·차후 세션에서)
...
- DB: 공용 MySQL(로컬·prod 동일). 마이그레이션 **0000~0007** 적용됨(이력 8행). 0008 은 생성됨(추가 전용: 테이블 2·`deleted_at` 2·enum widened 1·FK 4·인덱스 2)이나 **미적용** — 4-5a 작업 환경에 `.env` 가 없어 `bun run db:migrate` 를 못 돌렸다. 적용 후 이력 9행·테이블 34 를 확인한다. 마이그레이션이 컬럼을 지우면 이전 배포 코드가 깨지므로 적용과 push·배포를 연달아 한다(0007·0008 은 추가 전용이라 선택 적용 가능).
