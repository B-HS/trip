...
- [x] 12. 모델 예시 템플릿 — 타이베이 모델 일정, 시드, 앱 내 생성
...
- [ ] 4단계. 로드맵 7 Tiptap·YouTube(ADR-0027) → 6 커뮤니티·프로필(대문·사진, ADR-0028). **세션 3 진행 중**
    ...
    - [ ] 4-4. 로드맵 6(ADR-0028 + 구현 세부 ADR-0032)
        ...
        - [x] 4-4e. 지구본 확장(ADR-0035, 대응 커밋 `8b9fbd5` — origin/dev·prod 동기화, prod 배포 `dpl_8P8LFrBSrpJrA7NQQqxW1eNQqvYz` Ready, `trip.gumyo.net` 스모크 통과): **마이그레이션 0007 적용**(`trip_trip.departure_airport_code varchar(3) NULL`, 추가 전용, 이력 8행) · `AIRPORT_CODES`·`asAirportCode` · 템플릿 JSON 선택 필드(ICN 모델 예시 포함) · `PUBLIC_TRIP_CACHE_VERSION='4'` · 편집기 기본 정보 탭 `AirportCombobox`(생성 폼 미노출) · `/trips` 지구본 드래그 회전(`dragRotate`, 정밀 포인터 한정 OrbitControls) · 선 hover DOM 툴팁(`globe-interaction.ts` 의 `placeGlobeTooltip`·`GLOBE_ARC_STYLE`) · 선 클릭 `?route=` 필터 + 해제 셀(`collectGlobeRoutes`·`findGlobeRoute`·`filterTripsByRoute`, `shared/lib/search-param.ts` 공용). 브라우저 실측(A~F, 라이트·다크) → 지적 2건 중 히트 반경(should)을 ADR §5 대로 글로우 튜브 핸들러로 되돌려 수정 → 재확인 통과. 검증 typecheck·lint·`format:check`·`bun test` 506
    ...
    - [x] 4-5. 로드맵 6 확장(ADR-0037, ADR-0033 §3) — 완료·배포:
...
- [x] 7단계. 로드맵 4 AI(ADR-0029·0040): 자기 키 AES-GCM, 공식 provider 모델 조회·캐시·멤버십 검증, 영속 대화/작업/usage, Vercel Queues callback + 로컬 결정적 fallback, polling UI, 승인형 일정 diff, 전역 capability가 꺼진 환경의 AI UI 숨김·`/settings/ai` 404. 운영 `APP_ENCRYPTION_KEY`·Queues 설정과 provider smoke는 사용자 작업.
- [x] 8단계. 로드맵 10 SEO·GEO·JSON-LD·Analytics·Speed Insights(ADR-0030, hreflang·locale 별 sitemap 포함). 운영 지표 확인은 배포 후 작업.
- [x] 9단계. 개발자 API: ADR-0041, bearer token lifecycle, `/api/v1` owner-scoped endpoints, OpenAPI JSON, docs, rate-limit and auth tests. Migration 0012 follows 0010 consent and 0011 AI.
